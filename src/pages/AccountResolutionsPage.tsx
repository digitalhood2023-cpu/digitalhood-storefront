...
import { createResolution, getResolution, listResolutions, resolutionLabel, sendResolutionMessage, type OrderResolution, type ResolutionRequest, type ResolutionSummary } from '@/api/resolutions'
...
const input = 'w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-base text-slate-950 dark:border-slate-600 dark:bg-slate-950 dark:text-slate-100'
...
function RequestForm({ order, onCreated }: { order: AccountOrder; onCreated: (id: string) => void }) {
  ...
  return <form onSubmit={submit} className={`${panel} space-y-3`}>
    ...
    <div className="divide-y divide-slate-200 dark:divide-slate-700">{order.items?.map(item => <label key={item.id} className="flex items-center gap-3 py-2">
      <input type="checkbox" checked={Boolean(selected[item.id])} onChange={event => setSelected({...selected, [item.id]: event.target.checked ? 1 : 0})} disabled={busy} className="h-4 w-4 shrink-0 accent-[#29217d]" />
...
function CaseDetail({ item, refresh }: { item: OrderResolution; refresh: () => Promise<void> }) {
  ...
  return <div className="grid items-start gap-3 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)]">
    <section className={`${panel} min-w-0`}>
      ...
      {!['resolved', 'withdrawn'].includes(item.status) && <form onSubmit={send} className="mt-4 space-y-2"><label className="block text-xs font-semibold">Add information<textarea rows={2} maxLength={2000} required value={message} onChange={event => setMessage(event.target.value)} className={`${input} mt-1`} /></label>{error && <p role="alert" className="text-sm text-red-700 dark:text-red-300">{error}</p>}<Button type="submit" className="h-9" disabled={busy || !message.trim()}>{busy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <MessageSquare className="mr-2 h-4 w-4" />}Send reply</Button></form>}
    </section>
    <aside className="space-y-3"><section className={panel}><h2 className="text-sm font-bold">Selected items</h2><div className="mt-2 divide-y divide-slate-200 dark:divide-slate-700">{item.lines.map(line => <div key={line.line_id} className="flex justify-between gap-3 py-2 text-sm"><span>{line.name}<small className="block text-slate-500 dark:text-slate-400">Quantity {line.unit_numbers.length}</small></span><span className="shrink-0 font-semibold">{formatOrderMoney(String(Number(line.amount_minor) / 100), item.currency)}</span></div>)}</div><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Amounts paid for selected items; any shipping adjustment is reviewed separately.</p></section>
...
export default function AccountResolutionsPage() {
  const { orderId, resolutionId } = useParams(), navigate = useNavigate()
  ...
  const load = useCallback(async (more = false) => {
    ...
    try {
      if (resolutionId) {
        const response = await getResolution(resolutionId)
        if (run === generation.current) setDetail(response.resolution)
      } else {
        const [result, orderResult] = await Promise.all([listResolutions(orderId, more ? cursor || undefined : undefined), orderId && !more ? getCustomerOrder(orderId) : Promise.resolve(null)])
    ...
    } catch (cause) { if (run === generation.current) setError(cause instanceof Error ? cause.message : 'Unable to load resolutions.') }
    finally { if (run === generation.current) setBusy(false) }
  }, [isAuthenticated, orderId, resolutionId, cursor])
  const reload = useRef(load); reload.current = load
  useEffect(() => {
    setOrder(null); setDetail(null); setItems([]); setCursor(null)
    if (isAuthenticated) void reload.current()
    return () => { generation.current++ }
  }, [isAuthenticated, customer?.id, orderId, resolutionId])
  return <><Header /><main className="bg-slate-50 text-slate-950 dark:bg-slate-950 dark:text-slate-100"><div className="mx-auto max-w-6xl space-y-3 px-3 py-4">
    <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><Link to={orderId ? `/track-order/${orderId}` : '/account'} aria-label="Back"><ArrowLeft className="h-5 w-5" /></Link><h1 className="text-lg font-bold">{orderId ? `Resolve order #${orderId}` : 'Returns & resolutions'}</h1></div>{isAuthenticated && <button type="button" aria-label="Refresh cases" disabled={busy} onClick={() => void load()} className="rounded-lg border border-slate-300 p-2 dark:border-slate-600"><RefreshCw className={`h-4 w-4 ${busy ? 'animate-spin' : ''}`} /></button>}</div>
    {isLoading ? <p role="status" className="text-sm">Checking your account…</p> : !isAuthenticated ? <section className={panel}><p className="mb-2 text-sm">Sign in to manage requests linked to your purchases.</p><Button asChild><Link to="/login">Sign in</Link></Button></section> : <>
      ...
      {resolutionId && detail ? <CaseDetail key={detail.resolution_id} item={detail} refresh={() => load()} /> : <>
        ...
        {cursor && <Button variant="outline" disabled={busy} onClick={() => void load(true)}>Load more requests</Button>}
        ...
        {order && !error && <RequestForm key={`${customer?.id}:${order.id}`} order={order} onCreated={id => navigate(`/account/resolutions/${id}`)} />}
      </>}
    </>}
  </div></main><Footer /></>
}
