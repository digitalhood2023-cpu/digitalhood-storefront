...
function RefundOfferCard({id, offer, refresh}: {id: string; offer: RefundOffer; refresh: () => Promise<void>}) {
  ...
  return <section className={panel} aria-label="Refund offer"><h3 className="text-sm font-bold">{names[offer.status] || resolutionLabel(offer.status)}</h3><p className="mt-1 text-lg font-bold">{formatOrderMoney(String(Number(offer.amount_minor) / 100), offer.currency)}</p><p className="text-xs text-slate-500 dark:text-slate-400">Seller account {offer.seller_id} · Selected items</p><p className="mt-2 whitespace-pre-wrap break-words text-sm">{offer.note}</p>{offer.decision_note && <p className="mt-2 whitespace-pre-wrap break-words text-xs text-slate-600 dark:text-slate-300">Decision: {offer.decision_note}</p>}
